...
function QueueRow({
  ...
  const handleImagePathUpload = async (p: string) => {
    if (!p.trim()) return;
  ...
  return (
    ...
    >
      ...
      {/* Açıklama (Editable when editing) */}
      {editing && (
        <div className="flex flex-col col-span-2 lg:col-span-1 mt-2 lg:mt-0 min-w-0">
          <label className="text-[9px] uppercase tracking-wider text-zinc-600 font-bold mb-1">Açıklama</label>
          <textarea
            value={draft.description}
            onChange={(e) =>
              setDraft((d) => ({ ...d, description: e.target.value }))
            }
            className={cn(cellInput, "h-20 resize-none")}
            placeholder="İlan açıklaması..."
          />
        </div>
      )}

      {/* Görsel Yolu (Editable when editing) */}
      {editing && (
        <div className="flex flex-col col-span-2 lg:col-span-1 mt-2 lg:mt-0 min-w-0">
          <label className="text-[9px] uppercase tracking-wider text-zinc-600 font-bold mb-1">Görsel Yolu</label>
          <div className="flex gap-2">
            <div className="relative flex-1">
              <FolderOpen className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-zinc-600 pointer-events-none" />
              <input
                type="text"
                value={imagePathInput}
                onChange={(e) => setImagePathInput(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") { e.stopPropagation(); handleImagePathUpload(imagePathInput); } }}
                placeholder="/path/to/image.jpg"
                className={cn(cellInput, "pl-7 font-mono text-[10px]")}
              />
            </div>
            <button
              disabled={!imagePathInput.trim() || isUploadingImage}
              onClick={() => handleImagePathUpload(imagePathInput)}
              className="px-3 py-1.5 rounded-xl bg-zinc-800 text-zinc-300 text-[10px] font-bold hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            >
              {isUploadingImage ? <Loader2 className="w-3 h-3 animate-spin" /> : "Yükle"}
            </button>
          </div>
        </div>
      )}
...
export function PublishQueue({
  ...
  const filteredItems = searchQuery.trim()
    ? items.filter((item) =>
        [item.draft.name, item.draft.brand, item.draft.model, item.draft.storage, item.draft.town]
          .filter(Boolean)
  ...
  if (total === 0) {
    return (
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="flex-1 bg-[#0d1117] border border-white/5 lg:rounded-[24px] rounded-3xl flex flex-col items-center justify-center shadow-2xl font-sans p-12"
      >
        ...
        <h3 className="text-lg font-black text-zinc-400 uppercase tracking-wider mb-2">Kuyruk Boş</h3>
        <p className="text-sm text-zinc-600 text-center max-w-md mb-8 leading-relaxed">
          Henüz hiçbir ilan eklemediniz. Manuel ilan ekleyerek veya toplu ilan oluşturarak başlayabilirsiniz.
        </p>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#11F08E]/10 border border-[#11F08E]/20 text-[#11F08E] text-xs font-black uppercase tracking-widest">
            <Plus className="w-4 h-4" />
            Manuel Ekle
          </div>
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-500 text-xs font-black uppercase tracking-widest">
            <Layers className="w-4 h-4" />
            Toplu Ekle
          </div>
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-violet-500/10 border border-violet-500/20 text-violet-400 text-xs font-black uppercase tracking-widest">
            <Sparkles className="w-4 h-4" />
            Hızlı İlan
          </div>
        </div>
        <p className="text-[10px] text-zinc-700 mt-8 italic">
          Üst menüden ilan ekleme yöntemini seçebilirsiniz
        </p>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:items-center justify-between px-5 lg:px-6 py-5 border-b border-white/[0.03] bg-white/[0.01]">
        ...
        <div className="relative flex-1 max-w-xs mx-auto sm:mx-0">
          ...
          <input
            ...
            className="w-full bg-zinc-950/50 border border-zinc-800 rounded-xl pl-9 pr-3 py-2 text-xs text-zinc-300 outline-none focus:border-[#11F08E]/40 focus:ring-1 focus:ring-[#11F08E]/20 transition-all placeholder:text-zinc-700"
          />
        </div>
      ...
      {isRunning && (
        <div className="flex items-center gap-4 px-6 py-2 bg-white/[0.01] border-b border-white/[0.03]">
          <div className="flex-1 h-[2px] bg-white/[0.03] overflow-hidden rounded-full">
            <motion.div
              className="h-full bg-[#11F08E] rounded-full"
              initial={{ width: 0 }}
              animate={{ width: `${(done / total) * 100}%` }}
            />
          </div>
          <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap tabular-nums">
            {done} / {total}
          </span>
        </div>
      )}
