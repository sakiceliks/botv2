          ...
          <div className="grid gap-5 overflow-y-auto p-5 sm:grid-cols-[240px,minmax(0,1fr)] sm:p-6">
            <div className="space-y-4">
              <div className="overflow-hidden rounded-2xl border border-white/5 bg-[#111827]">
                ...
                <img
                  src={item.preview || item.draft?.imageUrl || ""}
                  alt={item.fileName}
                ...
                <tr key={item.id} className="align-top">
                  <td className="px-4 py-4">
                    <input
                      ...
                      aria-label={`${draft?.name ?? item.fileName} sec`}
                    />
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex min-w-[220px] items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 truncate text-xs text-zinc-500">
                          {draft?.productType || item.fileName}
                        </p>
                      </div>
                    </div>
                  </td>
...
export function BatchUploadPanel({
  ...
  function addPaths(raw: string) {
    const lines = raw
      .split("\n")
      .map((l) => l.trim())
      .filter((l) => l.length > 0);

    const validPaths = lines.filter((l) => {
      const ext = l.substring(l.lastIndexOf(".")).toLowerCase();
    ...
    for (let i = 0; i < snapshot.length; i++) {
      ...
      try {
        const res = await fetch("/api/analyze", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ imagePath: item.filePath }),
        });
        ...
        if (data.ok && data.draft) {
          ...
          current = current.map((it) =>
            it.id === item.id ? { ...it, status: "ready", draft, preview: previewUrl } : it,
          );
          setItems((prev) =>
            prev.map((it) =>
              it.id === item.id ? { ...it, status: "ready", draft, preview: previewUrl } : it,
    ...
    <div className="w-full">
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "upload" && (
          ...
          >
            {/* Path input area */}
            <div className="space-y-3 rounded-[24px] border-2 border-dashed border-zinc-800 p-6">
              ...
              <textarea
                ...
                className="w-full min-h-[120px] rounded-xl border border-zinc-800 bg-[#0d1117] px-3 py-3 text-xs font-mono text-zinc-200 outline-none focus:border-[#11F08E]/50 focus:ring-1 focus:ring-[#11F08E]/20 transition-all resize-y placeholder:text-zinc-600"
              />
              <button
                disabled={!pathInput.trim()}
                onClick={() => addPaths(pathInput)}
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-700 bg-zinc-800/60 py-2.5 text-xs font-semibold text-zinc-200 transition-all hover:border-zinc-500 hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-40"
              >
                ...
                {items.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-[10px] font-mono text-zinc-600">
                        {item.filePath}
                      </p>
                    </div>
                    <button
